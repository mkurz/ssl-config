...
private[sslconfig] class TracingX509ExtendedTrustManager(supplier: => X509ExtendedTrustManager, debug: SSLDebugConfig)(
  ...
  override def checkClientTrusted(chain: Array[X509Certificate], authType: String, socket: Socket): Unit = {
    tracer(
      "checkClientTrusted",
      Map("chain" -> chain, "authType" -> authType, "socket" -> socket),
      () => supplier.checkClientTrusted(chain, authType, socket)
    )
  }
  ...
  override def checkServerTrusted(chain: Array[X509Certificate], authType: String, socket: Socket): Unit = {
    tracer(
      "checkServerTrusted",
      Map("chain" -> chain, "authType" -> authType, "socket" -> socket),
      () => supplier.checkServerTrusted(chain, authType, socket)
    )
  }
  ...
  override def checkClientTrusted(chain: Array[X509Certificate], authType: String, engine: SSLEngine): Unit = {
    tracer(
      ...
      () => supplier.checkClientTrusted(chain, authType, engine)
    )
  }
  ...
  override def checkServerTrusted(chain: Array[X509Certificate], authType: String, engine: SSLEngine): Unit = {
    tracer(
      "checkServerTrusted",
      Map("chain" -> chain, "authType" -> authType, "engine" -> engine),
      () => supplier.checkServerTrusted(chain, authType, engine)
    )
  }
  ...
  override def checkClientTrusted(chain: Array[X509Certificate], authType: String): Unit = {
    tracer(
      "checkClientTrusted",
      Map("chain" -> chain, "authType" -> authType),
      () => supplier.checkClientTrusted(chain, authType)
    )
  }
  ...
  override def checkServerTrusted(chain: Array[X509Certificate], authType: String): Unit = {
    tracer(
      "checkServerTrusted",
      Map("chain" -> chain, "authType" -> authType),
      () => supplier.checkServerTrusted(chain, authType)
    )
  }
