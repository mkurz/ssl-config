 ...
 */
class CompositeX509KeyManager(mkLogger: LoggerFactory, keyManagers: Seq[X509KeyManager])
  ...
  def chooseClientAlias(keyType: Array[String], issuers: Array[Principal], socket: Socket): String = {
    logger.debug(
      s"chooseClientAlias: keyType = ${keyType.toSeq}, issuers = ${issuersToString(issuers)}, socket = $socket"
    )

    keyManagers.iterator
      .find { keyManager =>
        try {
      ...
      .flatMap(keyManager => Option(keyManager.chooseClientAlias(keyType, issuers, socket)))
      .orNull
  }

  override def chooseEngineClientAlias(keyType: Array[String], issuers: Array[Principal], engine: SSLEngine): String = {
    logger.debug(
      s"chooseEngineClientAlias: keyType = ${keyType.toSeq}, issuers = ${issuersToString(issuers)}, engine = $engine"
    )

    keyManagers.iterator
      .find { (keyManager: X509KeyManager) =>
        try {
          keyManager match {
      ...
      .flatMap(keyManager =>
        Option(keyManager.asInstanceOf[X509ExtendedKeyManager].chooseEngineClientAlias(keyType, issuers, engine))
      )
      .orNull
  }
  ...
  override def chooseEngineServerAlias(keyType: String, issuers: Array[Principal], engine: SSLEngine): String = {
    logger.debug(
      s"chooseEngineServerAlias: keyType = ${keyType.toSeq}, issuers = ${issuersToString(issuers)}, engine = $engine"
    )
    ...
    keyManagers.iterator
      ...
      .flatMap(keyManager =>
        Option(keyManager.asInstanceOf[X509ExtendedKeyManager].chooseEngineServerAlias(keyType, issuers, engine))
      )
      .orNull
  }
  ...
  def chooseServerAlias(keyType: String, issuers: Array[Principal], socket: Socket): String = {
    ...
    keyManagers.iterator
      ...
      .flatMap(keyManager => Option(keyManager.chooseServerAlias(keyType, issuers, socket)))
      .orNull
  }
  ...
  def getCertificateChain(alias: String): Array[X509Certificate] = {
    ...
    keyManagers.iterator
      ...
      .flatMap(keyManager => Option(keyManager.getCertificateChain(alias)))
      .orNull
  }
  ...
  def getPrivateKey(alias: String): PrivateKey = {
    ...
    keyManagers.iterator
      ...
      .flatMap(keyManager => Option(keyManager.getPrivateKey(alias)))
      .orNull
  }
