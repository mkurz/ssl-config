...
private[sslconfig] class TracingSSLContext(context: SSLContext, debugConfig: SSLDebugConfig)(
    implicit loggerFactory: LoggerFactory
) extends SSLContext(
      new TracingSSLContextSpi(context, debugConfig),
      context.getProvider,
      context.getProtocol
    )

private[tracing] class TracingSSLContextSpi(ctx: => SSLContext, debug: SSLDebugConfig)(
    implicit loggerFactory: LoggerFactory
  ...
  override def engineInit(
    ...
    tracer(
      "init",
      Map("keyManagers" -> keyManagers, "trustManagers" -> trustManagers, "secureRandom" -> secureRandom),
